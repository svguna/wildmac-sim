...
static int wildmac_sim()
{
    ...
    new_l1 = l1 = get_listen(0, epoch1, -1, -1);
    new_l2 = l2 = get_listen(1, epoch2, -1, -1);

    new_b1 = b1 = get_beacon(0, epoch1, l1, l1 + BEACON * SAMPLES);
    new_b2 = b2 = get_beacon(1, epoch2, l2, l2 + BEACON * SAMPLES);
    ...
    while (contact < 0 && n1 < EPOCH_LIMIT && n2 < EPOCH_LIMIT) {
        if (to_advance == 1) {
            ...
            n1++;
            new_l1 = get_listen(0, new_epoch1, -1, -1); 
            new_b1 = get_beacon(0, new_epoch1, new_l1, 
                    new_l1 + BEACON * SAMPLES);
        ...
        n2++;
        new_l2 = get_listen(1, new_epoch2, -1, -1);
        new_b2 = get_beacon(1, new_epoch2, new_l2, new_l2 + BEACON * SAMPLES);
