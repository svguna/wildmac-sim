//gcc -o simulation simulation_v2.c -Wall -I/opt/local/include -L/opt/local/lib -lgsl
//here it's added the possiblity of beaconing any time between [0,T], in case the full active time (listening and beaconing) overlaps with the next beacon, the active time will start inside the new epoch, in a time between [0, T-overlap]
#include <stdlib.h>
#include <stdio.h>
#include <stdint.h>
#include <string.h>
...
#define METRIC 0 // 0 GIVES THE CDF FOR THE FIRST DETECTION, 1: GIVES THE CDF FOR THE DISTANCE BETWEEN TWO DETECTIONS
#define MAX_EPOCHS 100
#define MAX_ITERATIONS 100
#define COLLISION_WAIT 400
#define SEPARATION_PROPORTION 20
#define SAMPLE_TIME 100
#define SLOT_TIME 250 //slot size used to calculate the number of cells of the array used to calculate the cumulative distriubted probability
#define TIMES 15 //used to definee the size of the array in base to the epoch size, which will be multiplied by TIMES
#define EPOCH_DELAY 5
#define CCA_POSITION 0 //0 the CCA is made at the begining, 1 the CCA is made at the end
#define CCA_LENGTH 250
...
struct t_conf {
	...
	int divide_actions; //0: no division, 1: decoupled, 2: listen, after beacon
	int phase;
	int active_type; //0: the active phase start in an epoch within [0, T-(L+S)], 1: the active phase starts in an epoch within (0,T), 2: Active phase starts within (0,T) but all inside the epoch, no overlaps, the overlapping part of the listen goes to the begining
	int real_detection;
};
...
int checkCcaOverlap(unsigned long beacon, unsigned long beacon_length, unsigned long listen, unsigned long listen_length)
{
	unsigned long ret, cca_start;
	int start, end, i, beacon_start_phase;
	
	if (CCA_POSITION == 0)
	{
		start=0;
		end=(listen_length/sensor_conf.beacon_t)-1;
		beacon_start_phase=0;
	}
	else 
	{
		start=1;
		end=listen_length/sensor_conf.beacon_t;
		beacon_start_phase=-CCA_LENGTH;
	}

	//printf("START %d END %d BEACON %lu LISTEN %lu\n",start,end, beacon, listen);
	
	for (i=start; i<=end; i++) 
	{
		cca_start = listen+(sensor_conf.beacon_t * i)+beacon_start_phase;
		ret = overlapCheck(beacon, beacon_length, cca_start, CCA_LENGTH);
		//printf("beacon_start %lu beacon_end %lu cca_start %lu cca_end %lu ret %lu\n", beacon, beacon + sensor_conf.beacon_t, cca_start, cca_start + CCA_LENGTH, ret);
		if (ret == CCA_LENGTH)
		{
			if ((instant_contact == 0) || (cca_start<instant_contact))
			{
				//printf("INSTANT_CONTACT A %lu\n", instant_contact);
				instant_contact = cca_start; //the instant of contact is defined by the event that happens the first
			}
			return TRUE;
		}
	}
...
//we check if they overlap for the beacon size, in this case the detection is guarantee. If not, we check if the cca overlaps with the beacon
void checkDetection(int sensor)
{
	unsigned long beacon, listen, ret, i;
	
	//we check if the previous listen or the current one of a node detects the new generated beacons
	
	
	//actual listen, previous beacon
	if (previous_beacon_matrix != NULL)
	{
		listen = listen_matrix[sensor] + sensor_conf.epoch_t;
		for (i=0; i<sensor_conf.n_sensors; i++)			
			if (sensor != i)
			{
				beacon = previous_beacon_matrix[i];
				if (sensor_conf.real_detection == TRUE) {
					ret = checkCcaOverlap(beacon, sensor_conf.beacon_t, listen, sensor_conf.listen_t);
				}
				else 
				{
					ret = checkCcaOverlap(beacon, sensor_conf.beacon_t + sensor_conf.listen_t, previous_beacon_matrix[sensor]+ sensor_conf.epoch_t, sensor_conf.listen_t + sensor_conf.beacon_t);
				}
	...
	//previous listen, current beacon
	if (previous_listen_matrix != NULL)
	{
		listen = previous_listen_matrix[sensor];
		for (i=0; i<sensor_conf.n_sensors; i++)			
			if (sensor != i)
			{
				beacon = beacon_matrix[i] + sensor_conf.epoch_t;
				if (sensor_conf.real_detection == TRUE) {
					ret = checkCcaOverlap(beacon, sensor_conf.beacon_t, listen, sensor_conf.listen_t);
				}
				else 
				{
					ret = checkCcaOverlap(beacon, sensor_conf.beacon_t + sensor_conf.listen_t, previous_beacon_matrix[i] + sensor_conf.epoch_t , sensor_conf.listen_t + sensor_conf.beacon_t);
				}
	...
	//current listen, current beacon
	listen = listen_matrix[sensor];
	for (i=0; i<sensor_conf.n_sensors; i++)			
		if (sensor != i)
		{
			...
			{
				ret = checkCcaOverlap(beacon, sensor_conf.beacon_t + sensor_conf.listen_t, beacon_matrix[sensor], sensor_conf.listen_t + sensor_conf.beacon_t);
			}
...
void generateBeaconData(int sensor, unsigned long init_time, unsigned long upper_limit, int current_epoch)
{
	...
	*/
	//we just generate full listens
		...
		if (beacon_matrix[sensor] != ULONG_MAX)
		{
			...
			{
				if ( sensor_conf.active_type == FALSE )
				{
...
void generateBeaconDecoupled(int sensor, unsigned long init_time, unsigned long upper_limit, int current_epoch)
{
	//int sensor;
	unsigned long separation, iterations;
	int regenerate;
	unsigned long init_listen, end_listen;
...
do
{
	...
	do
	{
		...
		regenerate=FALSE;
		listen_matrix[sensor] = init_time + (((double) gsl_rng_uniform (r) / 1 ) * (sensor_conf.epoch_t -init_time - upper_limit - separation));
		//we check if there is place to generate the listen in case it must be generated within [0,T] without overlapping the next epoch
		if(upper_limit!=0)
		{
			if ( (listen_matrix[sensor] < sensor_conf.beacon_t) && ( ((sensor_conf.epoch_t - (listen_matrix[sensor] + sensor_conf.listen_t)) < sensor_conf.beacon_t )))
				regenerate = TRUE;
		}
		iterations++;
	}while(regenerate);
	...
	//we check where we can generate the listen (left/rigth of the beacon)
	if (listen_matrix[sensor] < sensor_conf.beacon_t)
		init_listen = listen_matrix[sensor] + sensor_conf.listen_t;
	else 
		init_listen = delay[sensor] + init_time;
	
	if (upper_limit != 0) {
		 if (((sensor_conf.epoch_t + delay[sensor]) - (listen_matrix[sensor] + sensor_conf.listen_t)) < sensor_conf.beacon_t ) 
			 end_listen = listen_matrix[sensor] - 1; // if there is no  place on the right, the interval where it has to be generated it's before the beacon starts
		else
			end_listen = (delay[sensor] + sensor_conf.epoch_t) - sensor_conf.beacon_t; //if there is place, the interval ends at the end of the epoch
	}
	else {
		if ((listen_matrix[sensor] + sensor_conf.listen_t) > (sensor_conf.epoch_t + delay[sensor])) //the beaconing overlaps with next epoch, we generate the listen before the beaconing
			end_listen = listen_matrix[sensor] - 1; //if the beacon overlaps with the next epoch, the listen is created in the current epoch
		else
			end_listen =  sensor_conf.epoch_t + delay[sensor];//i the beacon is in the current epoch, the end time to generate is the end of the epoch
	}
	...
	do
	{
		...
		//we check if there is place to generate the listen in case it must be generated within [0,T] without overlapping the next epoch
		if (overlapCheck(beacon_matrix[sensor], sensor_conf.beacon_t, listen_matrix[sensor], sensor_conf.listen_t) != ULONG_MAX)
			regenerate = TRUE;
		iterations++;
	}while(regenerate);
	instant_contact = 0;//in this case we do not wan't to save the information
...
void generateListenBeaconData(int sensor, unsigned long init_time, unsigned long upper_limit, int current_epoch)
{
	...
	listen_matrix[sensor] = init_time + (((double) gsl_rng_uniform (r) / 1 ) * (sensor_conf.epoch_t -init_time - upper_limit));
	listen_matrix[sensor] = listen_matrix[sensor] + delay[sensor];
		
	if (listen_matrix[sensor] != ULONG_MAX)
	{
...
int main(int argc, char** argv)
{
	...
	if (argc != 11) {
		printf("ERROR: Not enough parameters\nsimulation N_SENSORS NUMBER_ITERATIONS EPOCH_T BEACON_T LISTEN_T MIN_NUM_DETECT DIVIDE_ACTIONS PHASE ACTIVE_TIME REAL_DETECTION - Time in microseconds\n");
		printf("argc %d\n", argc);
		exit(1);
	}
	...
	sensor_conf.active_type = atoi(argv[9]);
	sensor_conf.real_detection = atoi(argv[10]); 
	...
	for (iteration=0; iteration < sensor_conf.n_number_iterations; iteration++) 
	{
		...
		for (epoch=0; epoch < MAX_EPOCHS; epoch++) {
			...
			case 0:
					if ( sensor_conf.active_type == FALSE )
					{
						generateBeaconData(sensor, 0, sensor_conf.beacon_t+sensor_conf.listen_t, epoch);
					}
					else {
						
						if (previous_beacon_matrix == NULL) {
							generateBeaconData(sensor, 0, 0, epoch);
						}
						else
						{
							if ((previous_listen_matrix[sensor] + sensor_conf.listen_t) > (sensor_conf.epoch_t + delay[sensor])) {
								i = (previous_listen_matrix[sensor]+sensor_conf.listen_t - (sensor_conf.epoch_t + delay[sensor]) );
								generateBeaconData(sensor, i, 0, epoch);
							}
							else {
								generateBeaconData(sensor, 0, 0, epoch);
			...
			case 1:
					if ( sensor_conf.active_type == FALSE )
					{
						//generateBeaconDecoupled(sensor, 0, sensor_conf.beacon_t, epoch);
						generateBeaconDecoupled(sensor, 0, sensor_conf.listen_t, epoch);
					}
					else {
						
						if (previous_beacon_matrix == NULL) {
							generateBeaconDecoupled(sensor, 0, 0, epoch);
						}
						else
						{
							if ((previous_listen_matrix[sensor] + sensor_conf.listen_t) > (sensor_conf.epoch_t + delay[sensor])) {
								i = (previous_listen_matrix[sensor]+sensor_conf.listen_t - (sensor_conf.epoch_t + delay[sensor]) );
								generateBeaconDecoupled(sensor, i, 0, epoch);
							}
							else if ((previous_beacon_matrix[sensor] + sensor_conf.beacon_t) > (sensor_conf.epoch_t + delay[sensor])) {
								i = (previous_beacon_matrix[sensor]+sensor_conf.beacon_t - (sensor_conf.epoch_t + delay[sensor]) );
								generateBeaconDecoupled(sensor, i, 0, epoch);
							}
							else {
								generateBeaconDecoupled(sensor, 0, 0, epoch);
				...
				if ( sensor_conf.active_type == FALSE )
				{
					generateListenBeaconData(sensor, 0, sensor_conf.beacon_t+sensor_conf.listen_t, epoch);
				}
				else {
					...
					{
						if ((previous_beacon_matrix[sensor] + sensor_conf.beacon_t) > (sensor_conf.epoch_t + delay[sensor])) {
							i = (previous_beacon_matrix[sensor]+sensor_conf.beacon_t - (sensor_conf.epoch_t + delay[sensor]) );
			...
			//print's when the beacon and listen are generated coupled
			...
			//printf("%lu 1 %lu\n",  beacon_matrix[0]+(epoch*sensor_conf.epoch_t) + (sensor_conf.beacon_t/2), sensor_conf.beacon_t);
			//printf("%lu 1 %lu\n",  listen_matrix[0]+(epoch*sensor_conf.epoch_t) + (sensor_conf.listen_t/2), sensor_conf.listen_t);

			//printf("%lu 2 %lu\n",  beacon_matrix[1]+(epoch*sensor_conf.epoch_t) + (sensor_conf.beacon_t/2), sensor_conf.beacon_t);
			//printf("%lu 2 %lu\n",  listen_matrix[1]+(epoch*sensor_conf.epoch_t) + (sensor_conf.listen_t/2), sensor_conf.listen_t);
			//printf("\n");
			...
			if ( ((ret == TRUE) || (epoch >= (MAX_EPOCHS-1))) && (epoch >= EPOCH_DELAY))
			{
				...
				if (METRIC == 0)
				{
					//printf("INSTANT_CONTACT B %lu\n", instant_contact);
					instant_contact = instant_contact + (epoch-EPOCH_DELAY)*sensor_conf.epoch_t;				
					cdf[instant_contact/ SLOT_TIME ]++;
				
					iteration_result.detection_results[iteration] = (epoch-EPOCH_DELAY)+1; //to keep epochs between 1-MAX_EPOCHS
				...
				{
					second_detection = instant_contact + (epoch-EPOCH_DELAY)*sensor_conf.epoch_t;
					
					/*
					if( ((second_detection - first_detection) < 0) || ((second_detection - first_detection) > (sensor_conf.epoch_t*TIMES)) )
					{
						
						printf("FIRST %lu - SECOND DETECTION %lu - slots %lu - max slots %lu\n", first_detection, second_detection, (second_detection - first_detection)/ SLOT_TIME, (sensor_conf.epoch_t*TIMES)/SLOT_TIME);
						exit(0);
					}
					*/
					...
					//printf("FIRST %lu - SECOND DETECTION %lu - slots %lu\n", first_detection, second_detection, (second_detection - first_detection)/ SLOT_TIME);
					
					cdf[(second_detection - first_detection)/ SLOT_TIME ]++;
				...
				{
					first_detection = instant_contact + (epoch-EPOCH_DELAY)*sensor_conf.epoch_t;
					instant_contact = 0;
				}	
			}
			freeMatrix(previous_listen_matrix, previous_beacon_matrix);
